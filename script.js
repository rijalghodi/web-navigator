...
while (!finish) {
  ...
  // ------------------------------
  const response = prompt("Where would you go today? ").toLowerCase();

  if (response !== "n" && response !== "b" && response !== "q") {
    // we create a new page based on the url
    newPage(response);
  } else if (showNext === true && response === "n") {
    // we navigate forward a page
    nextPage();
  } else if (showBack === true && response === "b") {
